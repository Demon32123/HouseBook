...
export function BookDetail() {
  ...
  const [book, setBook] = useState<LibraryBookDTO>()
  ...
  const handleBorrowRequest = () => {
    loansService.createBorrow({ bookId: book.bookId, ownerUserId: ownerUser.id, message: "" })
    setBorrowSent(true);
      ...
      <div className="flex flex-col lg:flex-row gap-6 lg:gap-8">
        ...
        <div className="flex-1 min-w-0">
              ...
              <div className="flex gap-2">
                {(["unread", "reading", "read"] as ReadingStatus[]).map((s) => {
                  ...
                  return (
                    <button
                      key={s}
                      onClick={() => { setStatus(s); booksService.changeBookState(book.bookId, { readingStatus: s }).then( answ => console.log(answ) ) }}
          ...
          <div className="mt-6 grid grid-cols-2 sm:grid-cols-3 gap-4">
            <DetailItem icon={BookOpenCheck} label="Страниц" value={String(330)} />
            <DetailItem icon={Calendar} label="Год" value={String(book.year)} />
            <DetailItem icon={Hash} label="ISBN" value={book.isbn} />
            <DetailItem
              icon={Calendar}
              label="Добавлена"
              value={new Date(book.addedAt).toLocaleDateString("ru-RU")}
            />
          </div>
